...
{
    [Route("[controller]")]
    public class ApplicationRenewalController : Controller
    {
        
        public IActionResult Index()
        {
            var SubModeCode = "ApplicationRenewal";
            Task.Run(() => MenuLog.SaveMenuLog(Startup.Configuration, HttpContext, SubModeCode));
            return View();
        }

        [HttpPost("GET_APPLS_FOR_SearchAsync")]
        public async Task<IActionResult> GET_APPLS_FOR_SearchRenewalAsync([FromBody]SearchRenewalRequestBO requestBo)
        {
            SessionBO session = HttpContext.Session.GetObject<SessionBO>("UserSessionDetails");
            ApplicationRenewalDAL objDal = new ApplicationRenewalDAL(Startup.Configuration);
            
            using (DataTable dt = await objDal.GET_APPLS_FOR_SearchRenewalAsync(requestBo,session))
            {
        ...
        [HttpGet("GET_APPL_DTLS_FOR_RenewalAsync/{fdr_no}")]
        public async Task<IActionResult> GET_APPL_DTLS_FOR_RenewalAsync(string fdr_no)
        {
            SessionBO session = HttpContext.Session.GetObject<SessionBO>("UserSessionDetails");
            ApplicationRenewalDAL objDal = new ApplicationRenewalDAL(Startup.Configuration);
            SearchApplicationBO searchApplicationBO = new SearchApplicationBO();
            
            using (DataSet ds = await objDal.GET_APPL_DTLS_FOR_RENEWAsync(fdr_no, session))
            {
        ...
        [HttpGet, Route("GET_APPL_DTLS/{Appl_No}")]
