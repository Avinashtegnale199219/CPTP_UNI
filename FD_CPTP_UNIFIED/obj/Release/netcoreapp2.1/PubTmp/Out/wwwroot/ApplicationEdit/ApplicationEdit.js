...
const BindApplicationEditGrid = function ()
{
    try
    {
        ...
        ExtendedAjaxCall('ApplicationEdit/GET_APPLS_FOR_EDITAsync', null, 'GET', function (result)
        {
...
const EDIT = function (btn)
{
    try
    {
        ...
        ExtendedAjaxCall('ApplicationEdit/GET_APPL_DTLS_FOR_EDITAsync/' + Appl_No, null, 'GET', function (result)
        {
            try
            {
                    ...
                    if (!$.isEmptyObject(result.Table4) && result.Table4 != undefined && result.Table4.length > 0)
                    {
                        $.each(result.Table4, function (i, row)
                        {
                            if (row.Holder_Type == '01')
                            {
                                ...
                                if (BTP_OBJECT.FH_DATA_SOURCE == 'FRESH' || BTP_OBJECT.FH_DATA_SOURCE == "CKYC") {
                                    $("#FH_PEP").show();
                                    if (row.IsPEP != null || row.IsPEP != undefined) {

                                        var _IsPEP = row.IsPEP == false ? 'N' : 'Y';
                                        $('input[name="rdoInvPEP"][value=' + _IsPEP + "]")
                                            .prop("checked", true)
                                            .parent()
                                            .addClass("active");
                                    }
                                    ...
                                    if (row.IsPEP_Relative != null || row.IsPEP_Relative != undefined) {

                                        var _IsPEP_Relative = row.IsPEP_Relative == false ? 'N' : 'Y';
                                        $('input[name="rdoInvPEPRelated"][value=' + _IsPEP_Relative + "]")
                                            .prop("checked", true)
                                            .parent()
                                            .addClass("active");
                                    }
                            ...
                            if (row.Holder_Type == '02' && $('#chkSHApplicable').prop('checked'))
                            {
                                ...
                                if (BTP_OBJECT.SH_DATA_SOURCE == "FRESH" || BTP_OBJECT.SH_DATA_SOURCE == "CKYC") {
                                    $("#SH_PEP").show();
                                }
                                else {
                                    $("#SH_PEP").hide();
                                ...
                                if (BTP_OBJECT.SH_DATA_SOURCE == 'FRESH' || BTP_OBJECT.SH_DATA_SOURCE == "CKYC") {
                                    ...
                                    if (row.IsPEP != null || row.IsPEP != undefined) {
                                        var _IsPEP = row.IsPEP == false  ? 'N' : 'Y';
                                        $('input[name="rdoSHPEP"][value=' + _IsPEP + "]")
                                            .prop("checked", true)
                                            .parent()
                                            .addClass("active");
                                    }
                                    ...
                                    if (row.IsPEP_Relative != null || row.IsPEP_Relative != undefined) {
                                        var _IsPEP_Relative = row.IsPEP_Relative == false ? 'N' : 'Y';
                                        $('input[name="rdoSHPEPRelated"][value=' + _IsPEP_Relative + "]")
                                            .prop("checked", true)
                                            .parent()
                                            .addClass("active");
                                    }
                            ...
                            if ($('#chkSHApplicable').prop('checked') && row.Holder_Type == '03' && $('#chkTHApplicable').prop('checked'))
                            {
                                ...
                                if (BTP_OBJECT.TH_DATA_SOURCE == "FRESH" || BTP_OBJECT.TH_DATA_SOURCE == "CKYC") {
                                    $("#TH_PEP").show();
                                }
                                else {
                                    $("#TH_PEP").hide();
                                ...
                                if (BTP_OBJECT.TH_DATA_SOURCE == 'FRESH' || BTP_OBJECT.TH_DATA_SOURCE == "CKYC") {
                                    ...
                                    if (row.IsPEP != null || row.IsPEP != undefined) {
                                        ...
                                        var _IsPEP = row.IsPEP == false ? 'N' : 'Y';
                                        $('input[name="rdoTHPEP"][value=' + _IsPEP + "]")
                                            .prop("checked", true)
                                            .parent()
                                            .addClass("active");
                                    }
                                    ...
                                    if (row.IsPEP_Relative != null || row.IsPEP_Relative != undefined) {
                                        ...
                                        var _IsPEP_Relative = row.IsPEP_Relative == false ? 'N' : 'Y';
                                        $('input[name="rdoTHPEPRelated"][value=' + _IsPEP_Relative + "]")
                                            .prop("checked", true)
                                            .parent()
                                            .addClass("active");
                                    }
...
const FH_PAN_VERIFICATION_EDIT = function () {
