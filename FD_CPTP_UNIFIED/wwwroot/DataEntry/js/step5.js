...
$('input[name="rdoInvPEP"],input[name="rdoInvPEPRelated"],input[name="rdoSHPEP"],input[name="rdoSHPEPRelated"],input[name="rdoTHPEP"],input[name="rdoTHPEPRelated"]').click(function () {

    if ($(this).attr('value') == 'N') {
        if (!$(this).parent().hasClass('active')) {
            //$(this).parent('.Dv_AnsToggle').removeClass('active');
            //$(this).parents('.Dv_AnsToggle .lblOutr').child('.lblOutr').addClass('active');
            $(this).parents('.Dv_AnsToggle').children('.lblOutr').removeClass('active');
        ...
        //alert('Please declare politically exposed person?')
    }
    /*enableDisablebtnProceed();*/
});
...
const SaveStep5Details = function () {
    try {
        ...
        let isError = true;
        ExtendedAjaxCall('InvestorDetailsSave/SubmitStep5', InvestorDetailSaveStruct, 'POST', function (result) {
            try {
                ...
                else if (!$.isEmptyObject(result) && result.Status == 0 && !$.isEmptyObject(result.Msg) && !$.isEmptyObject(result.NSA_Res_Status)) {
                    isError = true;
                    // BtpMessagePopup(result[0].Msg, "error");//Commented 22-10-2024
...
const FH_GET_DATA_FOR_SAVE = function () {
    ...
    try {
        ...
        FH_DTLS.SchemeCode = $('#ddlSchemes').val().trim();

        FH_DTLS.IsInvPEP = $('input[name="rdoInvPEP"]:checked').val();
        FH_DTLS.IsInvPEPRelative = $('input[name="rdoInvPEPRelated"]:checked').val();
        FH_DTLS.IsInvOSV = $('input[name="rdoInvMWE"]:checked').val();
...
const SH_GET_DATA_FOR_SAVE = function () {
    ...
    try {
        ...
        if ($('input[name="rdoSHPEP"]:checked').val() != undefined) {
            SH_DTLS.IsInvPEP = $('input[name="rdoSHPEP"]:checked').val();
            SH_DTLS.IsInvPEPRelative = $('input[name="rdoSHPEPRelated"]:checked').val();
            SH_DTLS.IsInvOSV = $('input[name="rdoSHMWE"]:checked').val();
        }
...
const TH_GET_DATA_FOR_SAVE = function () {
    ...
    try {
        ...
        if ($('input[name="rdoTHPEP"]:checked').val() != undefined) {
            TH_DTLS.IsInvPEP = $('input[name="rdoTHPEP"]:checked').val();
            TH_DTLS.IsInvPEPRelative = $('input[name="rdoTHPEPRelated"]:checked').val();
            TH_DTLS.IsInvOSV = $('input[name="rdoTHMWE"]:checked').val();
        }
...
const FH_Save_Validations = function () {
    ...
    try {
        ...
        //FH PEP Details
        if ($('input[name="rdoInvPEP"]:checked').val() == undefined && (BTP_OBJECT.FH_DATA_SOURCE == 'FRESH' || BTP_OBJECT.FH_DATA_SOURCE == 'CKYC'))
            err_msg += "<p>Please provide PEP declaration..!</p>";

        if ($('input[name="rdoInvPEPRelated"]:checked').val() == undefined && (BTP_OBJECT.FH_DATA_SOURCE == 'FRESH' || BTP_OBJECT.FH_DATA_SOURCE == 'CKYC'))
...
const SH_Save_Validations = function () {
    ...
    try {
        ...
        //Avinash added pan verification changes
        if (BTP_OBJECT.SH_DATA_SOURCE.toUpperCase() == 'FRESH' || BTP_OBJECT.SH_DATA_SOURCE.toUpperCase() == 'CKYC') {
            if (SH_PAN_VERIFICATION()) {
            }
            else {
                err_msg += "<p>Please enter valid DOB as per PAN...!</p>";
            }
        }
        ...
        //SH PEP Details
        if ($('input[name="rdoSHPEP"]:checked').val() == undefined && (BTP_OBJECT.SH_DATA_SOURCE.toUpperCase() == 'FRESH' || BTP_OBJECT.SH_DATA_SOURCE.toUpperCase() == 'CKYC'))
            err_msg += "<p>Please provide Second holder's PEP declaration..!</p>";

        if ($('input[name="rdoSHPEPRelated"]:checked').val() == undefined && (BTP_OBJECT.SH_DATA_SOURCE.toUpperCase() == 'FRESH' || BTP_OBJECT.SH_DATA_SOURCE.toUpperCase() == 'CKYC'))
            err_msg += "<p>Please provide Second holder's PEP relative declaration..!</p>";
...
const TH_Save_Validations = function () {
    ...
    try {
        ...
        if (BTP_OBJECT.TH_DATA_SOURCE.toUpperCase() == 'FRESH' || BTP_OBJECT.TH_DATA_SOURCE.toUpperCase() == 'CKYC') {
            if (TH_PAN_VERIFICATION()) {
        ...
        //TH PEP Details
        if ($('input[name="rdoTHPEP"]:checked').val() == undefined && (BTP_OBJECT.TH_DATA_SOURCE.toUpperCase() == 'FRESH' || BTP_OBJECT.TH_DATA_SOURCE.toUpperCase() == 'CKYC'))
            err_msg += "<p>Please provide Third holder's PEP declaration..!</p>";

        if ($('input[name="rdoTHPEPRelated"]:checked').val() == undefined && (BTP_OBJECT.TH_DATA_SOURCE.toUpperCase() == 'FRESH' || BTP_OBJECT.TH_DATA_SOURCE.toUpperCase() == 'CKYC'))
            err_msg += "<p>Please provide Third holder's PEP relative declaration..!</p>";
...
//        if (!$.isEmptyObject(this.value) && IsValidPAN(this.value) && (BTP_OBJECT.SH_DATA_SOURCE == 'CKYC' || BTP_OBJECT.SH_DATA_SOURCE == 'FRESH')) {
//            let objReqBO = {};
...
//                    $('#Model_SH_PAN_VERIFICATION_Msg1').modal('show');
//                }
//                else {
...
//    })
//    .change(function () {
//        if (!$.isEmptyObject(this.value) && IsValidPAN(this.value) && (BTP_OBJECT.TH_DATA_SOURCE == 'CKYC' || BTP_OBJECT.TH_DATA_SOURCE == 'FRESH')) {
//            let objReqBO = {};
//            objReqBO.Source_Type = "CPTP_UNI";
...
//                }
//                else if (!$.isEmptyObject(result) && result.PAN_DOB_Match_Status == '0') {
//                    $('#Model_TH_PAN_VERIFICATION_Msg1').modal('show');
//                }
//                else {
...
//avinash added Occupation/sub - occupation changes
$('#DV_FH #ddlCustSegType').change(function () {
    debugger;
    var CustSegType_Code = $("#DV_FH #ddlCustSegType").val();
    ...
    ExtendedAjaxCall('DataEntry/Get_ddl_Occ_CustSegSubTypeList', CustSegType_Code, 'POST', function (result) {
        try {
            if (!$.isEmptyObject(result) && result.length > 0) {
                ...
                $('#DV_FH #ddlCustSegSubType').empty();
                $('#DV_FH #ddlCustSegSubType').append("<option value='select' data-row='0'>Select</option>");
                $.each(result, function (i, row) {
                    $('#DV_FH #ddlCustSegSubType').append("<option value='" + row['Code'] + "' data-row='" + JSON.stringify(row) + "'>" + row['Desc'] + "</option>");
                });
            }
            else {
...
$('#btn_SH_PAN_Match_No1').click(function () {
    $('#Model_SH_PAN_VERIFICATION_Msg1').modal('hide');
});
$('#btn_TH_PAN_Match_No1').click(function () {
    $('#Model_TH_PAN_VERIFICATION_Msg1').modal('hide');
});

const SH_PAN_VERIFICATION = function () {
    let isVerified = false;
    $('#preloader').show();
    try {
        if (!$.isEmptyObject($('#DV_SH #txtPAN').val()) && IsValidPAN($('#DV_SH #txtPAN').val()) && (BTP_OBJECT.SH_DATA_SOURCE == 'CKYC' || BTP_OBJECT.SH_DATA_SOURCE == 'FRESH')) {
            let objReqBO = {};
            objReqBO.Source_Type = "CPTP_UNI_DIGI";
            objReqBO.Holder_Type = '02';
            objReqBO.User_Name = $('#DV_SH #txtFName').val().trim() + ' ' + $('#DV_SH #txtMName').val().trim() + ' ' + $('#DV_SH #txtLName').val().trim();
            objReqBO.Remarks = "CPTP_UNI_DIGI";
            objReqBO.PAN_Holder_Name = $('#DV_SH #txtFName').val().trim() + ' ' + $('#DV_SH #txtMName').val().trim() + ' ' + $('#DV_SH #txtLName').val().trim();
            objReqBO.PAN_Holder_DOB = $('#DV_SH #txtDOB').val().trim();
            objReqBO.PAN_No = $('#DV_SH #txtPAN').val();
            objReqBO.SH_DATA_SOURCE = BTP_OBJECT.SH_DATA_SOURCE;
            ...
            ExtendedAjaxCall('PAN_Verification', objReqBO, 'POST', function (result) {
                if (!$.isEmptyObject(result) && result.PAN_DOB_Match_Status == '1' && result.Status == 'Success') {
                    ...
                    isVerified = true;
                    $('#Model_SH_NM_SCR_Msg').modal('show');
                }
                else if (!$.isEmptyObject(result) && result.PAN_DOB_Match_Status == '0') {
                    //$('#Model_SH_PAN_VERIFICATION_Msg1').modal('show');
                }
...
const TH_PAN_VERIFICATION = function () {
    let isVerified = false;

    try {
        if (!$.isEmptyObject($('#DV_TH #txtPAN').val()) && IsValidPAN($('#DV_TH #txtPAN').val()) && (BTP_OBJECT.TH_DATA_SOURCE == 'CKYC' || BTP_OBJECT.TH_DATA_SOURCE == 'FRESH')) {
            let objReqBO = {};
            objReqBO.Source_Type = "CPTP_UNI_DIGI";
            objReqBO.Holder_Type = '03';
            objReqBO.User_Name = $('#DV_TH #txtFName').val().trim() + ' ' + $('#DV_TH #txtMName').val().trim() + ' ' + $('#DV_TH #txtLName').val().trim();
            ...
            objReqBO.PAN_Holder_Name = $('#DV_TH #txtFName').val().trim() + ' ' + $('#DV_TH #txtMName').val().trim() + ' ' + $('#DV_TH #txtLName').val().trim();
            objReqBO.PAN_Holder_DOB = $('#DV_TH #txtDOB').val().trim();
            objReqBO.PAN_No = $('#DV_TH #txtPAN').val();
            objReqBO.TH_DATA_SOURCE = BTP_OBJECT.TH_DATA_SOURCE;
